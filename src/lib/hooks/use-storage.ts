...
// custom hook to set browser local/sync storage
  ...
  useEffect(() => {
    ...
    browser.storage.onChanged.addListener((changes, namespace) => {
      if (namespace === area && changes.hasOwnProperty(key)) {
        if (changes[key].newValue) setStoredValue(changes[key].newValue as T);
      }
    });
  }, []);
...
/**
 * Retrieves value from browser storage area
 *
 * @param key
...
/**
 * Sets object in browser storage area
 *
 * @param key
